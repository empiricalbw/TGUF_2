...
local TGUF_NONPLAYERPOLL_MASK = bit.bor(
                                ...
                                TGUF_THREAT,
                                TGUF_SPELL);
if (bit.bor(TGUF_NONPLAYEREVENT_MASK,TGUF_NONPLAYERPOLL_MASK) ~= TGUF_ALLFLAGS) then
    error("TGUF_NONPLAYER flags mismatch");
end

--[[
    This is the list of healing spells that we use for range testing on each class.
]]
local TGUF_HEALING_RANGE = { ["PRIEST"]  = "Heal",
                             ["DRUID"]   = "Healing Touch",
                             ["PALADIN"] = "Holy Light",
                             ["SHAMAN"]  = "Healing Wave"
                            };
...
--[[
    This is a template unit, used when the template editor is open so that the user can see
    various made-up stats.
]]
local TGUF_TEMPLATE_UNIT =
{
    ...
    isVisible      = 1,
    inHealingRange = 1,
    threat         = {isTanking=false,status=1,threatPct=90,rawThreatPct=110,threatValue=123400},
    buffs =
    {
        count = 32,
        buff =
...
--[[
    This creates a unit Listener object that will receive notifications when various
    events occur.  It is the responsibility of the caller to set callback functions for
    the various things they are interested in listening to.
]]
...
--[[
    This registers the specified listener object to listen for events from the specified unit.
    If the listener was previously listening for events from a different unit, a UnitChanged
    event is dispatched.
]]
function TGUnitComponent_RegisterUnitListener(listener,newUnit)
    -- If the listener was listening to an old unit, remove it from the old unit's list of
    -- listeners.
    ...
    if (oldUnit ~= newUnit) then
        --[[
        if (oldUnit ~= nil and newUnit ~= nil) then
            print("listener that was watching "..tostring(oldUnit.unit).." is now watching "..tostring(newUnit.unit));
        elseif (oldUnit == nil and newUnit ~= nil) then
            print("listener is now watching "..tostring(newUnit.unit));
        elseif (oldUnit ~= nil and newUnit ~= nil) then
            print("listener is no longer watching "..tostring(oldUnit.unit));
        end
        ]]
...
--[[
    Given a unit ID, this function will either return the table that maintains the
    stats for that particular unit, or create a new table if this unit is not being
    tracked yet.
]]
function TGUnitComponent_NewUnit(unit)
    ...
    theUnit.threat = {isTanking=nil,status=nil,threatPct=nil,rawThreatPct=nil,threatValue=nil};
...
--[[
    This function is called to have the specified listener start watching the specified unit,
    replacing any previous listener that was watching it.
]]
    ...
    for _,v in pairs(TGUF_CAST_LIST) do
        if (currTime > v.spell.endTime) then
            if (v.spell.type == "Casting") then
                TGUFUnitDebug("Generating fake UNIT_SPELLCAST_STOP for "..v.unit);
                TGUnitComponent_OnSpellcastStop("UNIT_SPELLCAST_STOP",v.unit);
            elseif (v.spell.type == "Channelling") then
                TGUFUnitDebug("Generating fake UNIT_SPELLCAST_CHANNEL_STOP for "..v.unit);
                TGUnitComponent_OnChannelStop("UNIT_SPELLCAST_CHANNEL_STOP",v.unit);
            else
                TGUFUnitDebug("Spell for unit "..v.unit.." ended, but it was an unknown cast type!");
            end
        end
    end
...
--[[
    Each listener for a particular unit can register to listen for notifications about
    any attribute change.  This registration is accomplished by inserting an object in
    the table for the attribute in question, X, that has a function of the name
    XUnitUpdate.  For instance, to listen for notifications about whether or not a unit
    is within healing range, you would do something like the following:
    
        obj.inHealingRangeUnitUpdate = MyInHealingRangeUpdateFunc;
        table.insert(listener.inHealingRange,obj);
    
    Now, when the inHealingRange event fires, your MyInHealingRangeUpdateFunc will be
    called with obj as the only parameter.
]]
...
function TGUnitComponent_UpdateUnit(unit,flags)
    ...
    if (btestmask(flags,TGUF_ISPLAYERTARGET)) then
        isPlayerTarget,isPlayerTargetChanged = TGUF_UpdateIsPlayerTarget(theUnit,newExists,existsChanged);
    end
    
    -- Update combo points
    local newComboPoints, comboPointsChanged;
    if (btestmask(flags,TGUF_COMBOPOINTS)) then
        newComboPoints, comboPointsChanged = TGUF_UpdateComboPoints(theUnit,newExists,existsChanged);
    end
            
    -- Update name
    local newName, nameChanged;
    if (btestmask(flags,TGUF_NAME)) then
        newName, nameChanged = TGUF_UpdateUnitName(theUnit,newExists,existsChanged);
    end
    
    -- Update class
    local newClass, newEnglishClass, classChanged;
    if (btestmask(flags,TGUF_CLASS)) then
        newClass, newEnglishClass, classChanged = TGUF_UpdateUnitClass(theUnit,newExists,existsChanged);
    end
    ...
    if (btestmask(flags,TGUF_HEALTH)) then
        newHealthCurrent, currentHealthChanged = TGUF_UpdateUnitHealth(theUnit,newExists,existsChanged);
        newHealthMax, maxHealthChanged = TGUF_UpdateUnitHealthMax(theUnit,newExists,existsChanged);
    ...
    if (btestmask(flags,TGUF_MANA)) then
        newManaType, manaTypeChanged = TGUF_UpdateUnitManaType(theUnit,newExists,existsChanged);
        newManaCurrent, manaCurrentChanged = TGUF_UpdateUnitMana(theUnit,newExists,existsChanged);
        newManaMax, manaMaxChanged = TGUF_UpdateUnitManaMax(theUnit,newExists,existsChanged);
    ...
    if (btestmask(flags,TGUF_LEVEL)) then
        newLevel, levelChanged = TGUF_UpdateUnitLevel(theUnit,newExists,existsChanged);
    end
    
    -- Update combat
    local newCombat, combatChanged;
    if (btestmask(flags,TGUF_COMBAT)) then
        newCombat, combatChanged = TGUF_UpdateUnitCombat(theUnit,newExists,existsChanged);
    end
    
    -- Update buffs
    local aBuffChanged, buffsChanged;
    if (btestmask(flags,TGUF_BUFFS)) then
        aBuffChanged, buffsChanged = TGUF_UpdateUnitBuffs(theUnit,newExists,existsChanged);
    end
    ...
    if (btestmask(flags,TGUF_DEBUFFS)) then
        aDebuffChanged,
        debuffsChanged,
        debuffCountsChanged = TGUF_UpdateUnitDebuffs(theUnit,newExists,existsChanged);
    end
    
    -- Update spellcast
    local isCasting, spellChanged;
    if (btestmask(flags,TGUF_SPELL)) then
        isCasting, spellChanged = TGUF_UpdateUnitSpell(theUnit,newExists,existsChanged);
    ...
    if (btestmask(flags,TGUF_REACTION)) then
        newreaction, reactionChanged = TGUF_UpdateUnitReaction(theUnit,newExists,existsChanged);
    end
    
    -- Update leader
    local newLeader, leaderChanged;
    if (btestmask(flags,TGUF_LEADER)) then
        newLeader, leaderChanged = TGUF_UpdateUnitIsGroupLeader(theUnit,newExists,existsChanged);
    end
    
    -- Update raid icon
    local newRaidIcon, raidIconChanged;
    if (btestmask(flags,TGUF_RAIDICON)) then
        newRaidIcon, raidIconChanged = TGUF_UpdateUnitRaidIcon(theUnit,newExists,existsChanged);
    end
    
    -- Update NPC
    local newNPC, npcChanged;
    if (btestmask(flags,TGUF_NPC)) then
        newNPC, npcChanged = TGUF_UpdateUnitIsNPC(theUnit,newExists,existsChanged);
    end
    ...
    if (btestmask(flags,TGUF_CLASSIFICATION)) then
        newClassification,
        classificationChanged = TGUF_UpdateUnitClassification(theUnit,newExists,existsChanged);
    end
    
    -- Update PVP status
    local newPVPStatus, pvpStatusChanged;
    if (btestmask(flags,TGUF_PVPSTATUS)) then
        newPVPStatus, pvpStatusChanged = TGUF_UpdateUnitPVPStatus(theUnit,newExists,existsChanged);
    end
    
    -- Update living status
    local newLiving, livingChanged;
    if (btestmask(flags,TGUF_LIVING)) then
        newLiving, livingChanged = TGUF_UpdateUnitLiving(theUnit,newExists,existsChanged);
    end
    
    -- Update tapped status
    local newTapped, tappedChanged;
    if (btestmask(flags,TGUF_TAPPED)) then
        newTapped, tappedChanged = TGUF_UpdateUnitTapped(theUnit,newExists,existsChanged);
    end
    
    -- Update visible status
    local newIsVisible, isVisibleChanged;
    if (btestmask(flags,TGUF_ISVISIBLE)) then
        newIsVisible, isVisibleChanged = TGUF_UpdateUnitIsVisible(theUnit,newExists,existsChanged);
    end
    ...
    if (btestmask(flags,TGUF_INHEALINGRANGE)) then
        newInHealingRange,
        inHealingRangeChanged = TGUF_UpdateUnitInHealingRange(theUnit,newExists,existsChanged);
    end
    ...
    if (btestmask(flags,TGUF_CREATURETYPE)) then
        newCreatureType,
        creatureTypeChanged = TGUF_UpdateUnitCreatureType(theUnit,newExists,existsChanged);
    end
    ...
    if (debuffCountsChanged) then
        TGUFUnitDebug(unit..".debuffCounts->"..tostring(theUnit.debuffCounts.Magic)..","..tostring(theUnit.debuffCounts.Curse)..
                    ","..tostring(theUnit.debuffCounts.Disease)..","..tostring(theUnit.debuffCounts.Poison));
    ...
    if (livingChanged) then
        ...
        -- Sometimes we lose unit health update events when a unit comes alive or dies.
        -- We do an update here.  I think this is a bug in Blizzard's engine.
    ...
    if (inHealingRangeChanged) then
        TGUFUnitDebug(unit..".inHealingRange.inHealingRange->"..tostring(newInHealingRange));
...
local TGUF_LOOT_MASTER_ID = {[0]="player",[1]="party1",[2]="party2",[3]="party3",[4]="party4"};
...
local unitPetMap = {    ["player"]="pet", ["party1"]="partypet1", ["party2"]="partypet2", ["party3"]="partypet3", ["party4"]="partypet4",
                        ...
                        ["raid21"]="raidpet21", ["raid22"]="raidpet22", ["raid23"]="raidpet23", ["raid24"]="raidpet24", ["raid25"]="raidpet25", ["raid26"]="raidpet26", ["raid27"]="raidpet27", ["raid28"]="raidpet28", ["raid29"]="raidpet29", ["raid30"]="raidpet30",
                        ["raid31"]="raidpet31", ["raid32"]="raidpet32", ["raid33"]="raidpet33", ["raid34"]="raidpet34", ["raid35"]="raidpet35", ["raid36"]="raidpet36", ["raid37"]="raidpet37", ["raid38"]="raidpet38", ["raid39"]="raidpet39", ["raid40"]="raidpet40"
                    };
...
function TGUnitComponent_OnSpellcastUpdate(event,unit)
    ...
    if (theUnit.spell.spell) then
        ...
        TGUF_CAST_LIST[theUnit.unit] = nil;
        TGUFWarning("Get spellcast started event for unit "..unit.." but UnitCastingInfo returned nil!");
    end
    TGUnitComponent_MarkUpdate("spell",theUnit);
end
...
function TGUnitComponent_OnSpellcastStop(event,unit)
    ...
    if (theUnit.spell.type ~= nil) then
        if (theUnit.spell.type == "Casting") then
            ...
            TGUFWarning("Got spellcast stop event for unit "..unit.." but was "..theUnit.spell.type..".");
        end
    else
        TGUFUnitDebug("Got spellcast stop event for unit "..unit.." but no spell was in progress.");
    end
end
...
function TGUnitComponent_OnChannelUpdate(event,unit)
    ...
    if (theUnit.spell.spell) then
        ...
        theUnit.spell.type = nil;
        TGUF_CAST_LIST[theUnit.unit] = nil;
        TGUFUnitDebug("Get channelling started event for unit "..unit.." but UnitChannelInfo returned nil!");
    end
    TGUnitComponent_MarkUpdate("spell",theUnit);
end
...
function TGUnitComponent_OnChannelStop(event,unit)
    ...
    if (theUnit.spell.type ~= nil) then
        if (theUnit.spell.type == "Channelling") then
            ...
            TGUFUnitDebug("Got channel stop event for unit "..unit.." but was "..theUnit.spell.type..".");
        end
    else
        TGUFUnitDebug("Got channel stop event for unit "..unit.." but no spell was in progress.");
    end
end
